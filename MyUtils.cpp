...
/**
  Calculate ration of dark to light pixels in framebuffer,
  using pixels from center area
  Pixel data is packed (two pixels per byte)
*/
float calcPartialDarkRatio(uint8_t *framebuffer, uint8_t margin)
{
  ...
  float ratio = (float)dark / ((EPD_WIDTH - margin - margin) * (EPD_HEIGHT - margin - margin));
  Serial.printf("margin %d, dark: %d, ratio: %.2f\n", margin, dark, ratio);
  return ratio;
}
